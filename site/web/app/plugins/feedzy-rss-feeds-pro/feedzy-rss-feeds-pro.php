 ...
 * Description:         FEEDZY RSS Feeds Premium extends the functionality of FEEDZY RSS Feeds Lite.
 * Version:             3.1.0
 * Author:              Themeisle
 * Author URI:          https://themeisle.com
 * Text Domain:         feedzy-rss-feeds
 * Domain Path:         /languages
 * Requires Plugins:    feedzy-rss-feeds
 ...
 * 
 * @param string $php_class The PHP class to load.
 * 
 * @return bool
 *
 * @since    1.0.0
 */
function feedzy_rss_feeds_pro_autoload( $php_class ) {
	$namespaces = array( 'Feedzy_Rss_Feeds_Pro' );
	foreach ( $namespaces as $namespace ) {
		if ( substr( $php_class, 0, strlen( $namespace ) ) === $namespace ) {
			$filename = plugin_dir_path( __FILE__ ) . 'includes/' . str_replace( '_', '-', strtolower( $php_class ) ) . '.php';
			if ( is_readable( $filename ) ) {
				require_once $filename;

				return true;
			}
			$filename = plugin_dir_path( __FILE__ ) . 'includes/abstract/' . str_replace( '_', '-', strtolower( $php_class ) ) . '.php';
			if ( is_readable( $filename ) ) {
				require_once $filename;

				return true;
			}
			$filename = plugin_dir_path( __FILE__ ) . 'includes/admin/' . str_replace( '_', '-', strtolower( $php_class ) ) . '.php';
			if ( is_readable( $filename ) ) {
				require_once $filename;

				return true;
			}
			$filename = plugin_dir_path( __FILE__ ) . 'includes/public/' . str_replace( '_', '-', strtolower( $php_class ) ) . '.php';
			if ( is_readable( $filename ) ) {
				require_once $filename;

				return true;
			}
			$filename = plugin_dir_path( __FILE__ ) . 'includes/admin/services/' . str_replace( '_', '-', strtolower( $php_class ) ) . '.php';
			if ( is_readable( $filename ) ) {
				require_once $filename;

				return true;
			}
			$filename = plugin_dir_path( __FILE__ ) . 'includes/elementor/' . str_replace( '_', '-', strtolower( $php_class ) ) . '.php';
			if ( is_readable( $filename ) ) {
				require_once $filename;
...
function run_feedzy_rss_feeds_pro() {
	...
	define( 'FEEDZY_PRO_ABSPATH', __DIR__ );
	define( 'FEEDZY_PRO_DIRNAME', basename( FEEDZY_PRO_ABSPATH ) );
	if ( ! defined( 'FEEDZY_ROOT_API' ) ) {
	...
	define( 'FEEDZY_PRO_VERSION', '3.1.0' );
	...
	add_filter(
		'themeisle_sdk_compatibilities/' . FEEDZY_PRO_DIRNAME,
	...
	add_filter(
		...
		function ( $metadata ) {
			...
			$support_link = tsdk_support_link( FEEDZY_PRO_BASEFILE );
			if ( ! empty( $support_link ) ) {
				$metadata['premium_support_link'] = $support_link;
			}

			$metadata['feature_request_link'] = 'https://app.formbricks.com/s/clvjy6p910gsv12l2xw3ub2q0';
	...
	add_filter( 'feedzy-rss-feeds-pro_sdk_enable_private_translations', '__return_true' );

	add_filter(
		'feedzy_rss_feeds_pro_lc_no_valid_string',
		function ( $message ) {
			return str_replace( '<a href="%s">', '<a href="' . admin_url( 'admin.php?page=feedzy-support&tab=license' ) . '">', $message );
		}
	);

	add_filter( 'feedzy_rss_feeds_pro_hide_license_field', '__return_true' );
}

/**
 * Registers with the SDK
 * 
 * @param string[] $products The registered products.
 * 
 * @return string[]
 *
 * @since    1.0.0
